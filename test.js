//THIS DOCUMENT IS USED TO TEST GITHUB COPILOT

let testa = 'HDHRDT';
console.log(testa)
console.log(OrderMods(testa));
let testb = testa.replace(/(.{2})/g, "$1,"); //this is a preferred way of adding a letter every x characters
console.log(testb)
...
//recreate the function above but instead it subtracts the mods from the total
...
//make a function that grabs a string and orders it by the order of the mods in the array
...
//make a function that takes a string and returns the number of mods in the string
function ModCount(modString) {
    let modStringArray = modString.replace(/(.{2})/g, "$1 ").split(' ')
    let modCount = 0
    for (let i = 0; i < modStringArray.length; i++) {
        if (modStringArray[i].length === 2) {
            modCount++
        }
    }
    return modCount
