 ...
 */
export function msToOD(hitwindow300: number, hitwindow100?: number, hitwindow50?: number) {
    let od: string = 'NaN';
 ...
 */
export function msToAR(ms: number): types.ApproachRate {
    let ar: number;
    if (ms < 300) {
        ar = 11;
    }
...
export function modded(stats: {
        ...
        if (!customSpeed || customSpeed == 1) {
            return {
                ...
                extra: {
                    ...
                    arMs: ARtoms(stats.ar),
                    odMs: ODtoms(stats.od),
                }
            };
        }
        const modAr = ARtoms(stats.ar) / customSpeed;
        const modOd = ODtoms(stats.od);
    ...
    let modCs = stats.cs;
    let modArMs = ARtoms(stats.ar);
    let modOdMs = ODtoms(stats.od);
    let modHp = stats.hp;
    ...
    if (typeof mods[0] == 'string') {
        ...
        if (mods.includes('HR')) {
            modCs *= 1.3;
            modArMs = ARtoms(stats.ar * 1.4);
            modOdMs = ODtoms(stats.od * 1.4);
            modHp *= 1.4;
        } else if (mods.includes('EZ')) {
            modCs /= 2;
            modArMs = ARtoms(stats.ar / 2);
            modOdMs = ODtoms(stats.od / 2);
            modHp /= 2;
        }
    ...
    return {
        cs: stats.cs,
        ar: msToAR(modArMs),
        od: msToOD(modOdMs.hitwindow_300),
        hp: stats.hp,
        bpm: modBpm,
        song_length: modSl,
        extra: {
