...
function odDT(od: number) {
    let range300: number = (79 - (od * 6) + 0.5) * 2 / 3
    let odobj = {
        hitwindow_300: range300,
        hitwindow_100: (139 - (od * 8) + 0.5) * 2 / 3,
        hitwindow_50: (199 - (od * 10) + 0.5) * 2 / 3,
        od_num: Math.abs(((79.5 - range300) / 6)) > 11 ? 11 : Math.abs(((79.5 - range300) / 6)).toFixed(2),
    }
...
function odHT(od: number) {
    let range300: number = (79 - (od * 6) + 0.5) * 4 / 3
    let odobj = {
        hitwindow_300: range300,
...
function calcgrade(hit300: number, hit100: number, hit50: number, miss: number) {
    let totalhits: number = hit300 + hit100 + hit50 + miss
